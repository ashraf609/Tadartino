...
import { GET_ALL_REQUESTS_FAIL, GET_ALL_REQUESTS_REQUEST, GET_ALL_REQUESTS_SUCCESS,GET_REQUEST_REQUEST,GET_REQUEST_SUCCESS,GET_REQUEST_FAIL, ADD_REQUEST_REQUEST, ADD_REQUEST_SUCCESS, ADD_REQUEST_FAIL, DELETE_REQUEST_REQUEST, DELETE_REQUEST_SUCCESS, DELETE_REQUEST_FAIL, UPDATE_REQUEST_REQUEST, UPDATE_REQUEST_SUCCESS, UPDATE_REQUEST_FAIL } from '../Constants/RequestConstants'

//get all REQUESTs
export const get_all_requests_action =(token) => async dispatch =>{
    dispatch({type:GET_ALL_REQUESTS_REQUEST})
    try {
        const {data} = await axios.get("/api/REQUEST/get-all-REQUESTs",{
            headers:{
                token
            }
        });
...
export const get_request_action =(token,id) => async dispatch =>{
    ...
    try {
        const {data} = await axios.get("/api/request/get-request",{
            headers:{
                token
            },
            params:{
...
export const add_request_action =(token,info) => async dispatch =>{
    ...
    try {
        const {data} = await axios.post("/api/request/create-request",info,{
            headers:{
                token
            }
        });
...
export const update_request_action =(token,info) => async dispatch =>{
    ...
    try {
        const {data} = await axios.post("/api/request/update-request",info,{
            headers:{
                token
            }
        });
...
export const delete_request_action =(token,id) => async dispatch =>{
    ...
    try {
        const {data} = await axios.delete("/api/request/delete-request",{
            headers:{
                token
            },
            params:{
